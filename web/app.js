...
// note: needs to be done in the 'pagerendered' event
                ...
                /*   var screenRect = pageView.viewport.convertToViewportRectangle(pdfRect);
                 ...
                 var y = Math.min(screenRect[1], screenRect[3]), height = Math.abs(screenRect[1] - screenRect[3]);
                 */
                var bubblediv = document.createElement('bubble' + i);
                bubblediv.style.backgroundColor = "rgba(229,255,224,1)";
                bubblediv.style.position = "absolute";
                bubblediv.style.borderStyle = "double";
                ...
                //  bubblediv.style.width = '200px';
                // bubblediv.style.height = '20px';
                bubblediv.style.fontSize = 8 * pageView.viewport.fontScale + 'px';
                bubblediv.style.fontFamily = "gill";

                bubblediv.innerHTML = references[i].text;
    ...
    function getReferences() {
         $.ajax({url: "http://localhost:8080/ParserServer-1.0-SNAPSHOT/getref", success: function(result){
                console.log(result);
         }});
